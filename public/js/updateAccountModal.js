  ...
  submitButton.addEventListener('click', async (event) => {
    try {
      ...
      if (account) {
        ...
        updatedElement.querySelector('#account-server-name').textContent = account.server.name;
        updatedElement.querySelector('.article__logo-server').src = `/images/${account.server.img}`;
        updatedElement.querySelector('.article__logo-server').alt = `image du serveur ${account.server.name}`;
...
export default function initModalUpdateAccount() {
  openModal();
  closeModal();
  submitUpdateAccount();
}
